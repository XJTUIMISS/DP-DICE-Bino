...
void CP::init_offline_data(){
    ...
    for(int i=0; i<nthreads; i++){
        ...
        ss << PREP_DATA_PREFIX << "/RandExps-p-64-P" << mynum;
        if (i){
            ss << "-" << i;
        }
    ...
    switch(d){
        ...
        case 17:
            if(poly_cdb_f[d].empty()){
                throw runtime_error("The polynomial function has not been initialized 1");
            }else{
                return poly_cdb_f[d];
            }
            break;
        case 8:
            if(clookup_d3.empty()){
                throw runtime_error("The polynomial function has not been initialized 2");
            }else{
                return clookup_d3;
            }
            break;
        case 4:
            if(clookup_d2.empty()){
                throw runtime_error("The polynomial function has not been initialized 3");
            }else{
                return clookup_d2;
            }
            break;
        default:
            throw runtime_error("The polynomial function has not been initialized 4");
    }
    throw runtime_error("The CEXtract polynomial function has not been initialized 5");
...
void CP::benchmark(){
    ...
    */
    Share<gfp> Binomial_noise, temp_noise;
    int i, Bino_scale = 2723094, half_Bino_scale = 1361547;
    Binomial_noise.assign_zero();
    for (i = 0; i < Bino_scale; ++i)
    {
        temp_noise.input(rand2_pool[0], false);
        Binomial_noise += temp_noise;
    }
    gettimeofday(&t1, NULL);
    multi_thread_bucket_bExtractZ(share_BFS,share_Z);
    share_Z = share_Z * 10 + Binomial_noise;
    if (mynum)
    {
        share_Z.sub(share_Z, half_Bino_scale, false, alphai);
    }
    else
    {
        share_Z.sub(share_Z, half_Bino_scale, true, alphai);
    }
    gettimeofday(&t2, NULL);
    cpu_time_used = (double)(t2.tv_sec-t1.tv_sec)*1000+(double)(t2.tv_usec-t1.tv_usec)/1000;
    //printf("bExtractZ\n%d\t%d\t%f\n",uN,oM,cpu_time_used);
...
template <class T>
double CP::fm_approximateCount(Share<T>& Z, int M){
    const double PHI = 0.77351;
    // Share<T> Binomial_noise, temp_noise;
    // Binomial_noise.assign_zero();
    // T a_bino, mac_bino;
    // bigint tmp_bino;
    // int Bino_scale = 2723094, i;
    // for (i = 0; i < Bino_scale; ++i)
    // {
    //     temp_noise.input(rand2_pool[0], false);
    //     Binomial_noise += temp_noise;
    // }
    // // open_and_check(Binomial_noise, a_bino, mac_bino);
    // // to_bigint(tmp_bino, a_bino);
    // // double R_bino = tmp_bino.get_si();
    // // cout << "----------!!!!!!!!!!!!!!!!!!!----------------" <<  R_bino << "----------!!!!!!!!!!!!!!!!!!!----------------" << endl;
    T a,mac;
    bigint tmp;
    // Z += Binomial_noise;
...
bool DP_connector::data_collection(int socket_num, std::vector<std::vector<Share<gfp>>>& share_FS){
    bool ret = true;
    int w = ceil(log2((double)uN / (double)oM))+6;
