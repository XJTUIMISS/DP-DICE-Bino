...
double CP::fm_approximateCount(Share<T>& Z, int M){
    ...
    T a,mac;
    Share<T> Distributed_Gaussian_noise;
    Distributed_Gaussian_noise = share_FS[oM][0];
    Z += Distributed_Gaussian_noise;
    bigint tmp;
...
bool DP_connector::data_collection(int socket_num, std::vector<std::vector<Share<gfp>>>& share_FS){
    ...
    share_FS.resize(oM + 1);
    for(int m=0; m<oM; m++){
        share_FS[m].resize(w);
    }
    share_FS[oM].resize(1);
...
bool DP_connector::share_value(std::vector<std::vector<Share<T>>>& des_vec, int socket_num){
    ...
    y_value.unpack(share_stream);
    a_mac = a_Array[0].get_mac();
    a_value = a_Array[0].get_share();
    tmp.assign_zero();
    if(mynum == 0){
        a_value.add(y_value);
        tmp.mul(y_value,alphai);
    ...
    des_vec[ofs_M][0].set_share(a_value);
    des_vec[ofs_M][0].set_mac(a_mac);
